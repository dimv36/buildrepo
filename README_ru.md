# buildrepo
Скрипт сборки APT репозиториев на Debian-like ОС
...
Опция `--boot` выполняет запуск контейнера, вызывая полный запуск ОС контейнера, в частности, в контейнере необходимо установить менеджер `systemd` или `init.d`.
...
По умолчанию при сборке пакетов используется уже распакованный chroot. Если требуется собирать каждый пакет в chroot'е, предварительно очистив его, то используется опция `--clean` - перед сборкой каждого пакета удаляется распакованный образ chroot'а, распаковывается из заранее созданного и выполняется сборка.
